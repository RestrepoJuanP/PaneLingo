...
- **Invalidación de sesiones al cambiar la contraseña:** la garantiza el propio Django, no código nuestro. La sesión guarda un HMAC derivado del hash de la contraseña y `get_user()` lo comprueba en cada petición, así que al cambiarla toda sesión anterior deja de validar. Está cubierta por `OtherSessionsTests` en `accounts/tests/test_password_reset.py`. **No la reimplementes ni la desactives:** si tocas el backend de sesiones o `get_session_auth_hash`, esa prueba debe seguir pasando.
- **Caducidad del enlace de recuperación:** `PASSWORD_RESET_TIMEOUT = 3600` (una hora), frente a los tres días que trae Django por defecto. El motivo está comentado en `config/settings.py`.
...
- **Chip de estado** (solo lectura: "Completado", "Borrador", "En progreso"): `border-radius: var(--r-pill)`, `padding: var(--pad-chip)`, peso 700, tamaño `--fs-badge`, con la pareja fondo/texto de la tabla de colores semánticos.
- **Pill selector** (control de elección: idioma, filtro, rol, orden de lectura): misma forma, `padding: var(--pad-filter)`, pero **paleta distinta**. En reposo `--pill-bg` / `--pill-fg` / `--pill-border`; seleccionado se pinta de negro con `--pill-selected-bg` / `--pill-selected-fg`. No uses el índigo de marca ni los colores semánticos aquí. La **tarjeta de opción** (bloque con título y descripción) sí usa índigo al seleccionarse: borde `--brand` sobre `--brand-tint-2`.
